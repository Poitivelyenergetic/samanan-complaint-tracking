...
import { subscribeToTicketSources, deleteTicketSource } from "@/lib/ticketSources";
import { useAuth } from "@/lib/auth-context";
import { hasPermission, localizedName, type TicketSource } from "@/lib/types";

export default function TicketSourcesPage() {
  const t = useTranslations("ticketSources");
  const tCommon = useTranslations("common");
  const locale = useLocale();
  const { profile } = useAuth();
  const canCreate = hasPermission(profile, "ticketSources", "create");
  ...
  useEffect(() => subscribeToTicketSources(setSources), []);

  async function handleDelete(source: TicketSource) {
    if (!window.confirm(t("deleteConfirm"))) return;
    await deleteTicketSource(source.id);
  }
              ...
              sources.map((source) => (
                <tr key={source.id} className="border-b border-border last:border-0 hover:bg-black/[0.02]">
                  <td className="px-4 py-3">
                    {canUpdate ? (
                      <Link href={`/ticket-sources/${source.id}`} className="font-medium text-foreground hover:text-brand">
